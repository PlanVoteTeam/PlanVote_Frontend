    ...
    async function handleUpdateDurationEvent(
        ...
        try {
            const response = await fetch(apiUrl + `events/${eventId}`, {
                ...
                body: JSON.stringify({ minDuration: minDuration, maxDuration: maxDuration }),
            });
            if (!response.ok) {
                throw new Error(
        ...
        <div>
            ...
            <div>
                <form
                    ...
                    e.preventDefault();
                    if (eventId)
                        handleUpdateDurationEvent(eventId, eventDurationMin, eventDurationMax);
                    }}
                >
                    <div className="field has-addons is-align-items-flex-end">
                        <div className="control input-width">
                            ...
                            <input
                                ...
                                onChange={(e) => setEventDurationMin(parseInt(e.target.value))}
                            />
                        </div>
                        <div className="control input-width">
                            ...
                            onChange={(e) => setEventDurationMax(parseInt(e.target.value))}
                            />
                        </div>
            ...
            <div>
                <a
                    ...
                    onClick={handleEditEventDurationClick}
                >
                    {/* If event description is blank, display a message */}
                    {eventObject && eventObject.minDuration && eventObject.maxDuration
                    ? "🖋️ Durée du voyage : " + eventObject.minDuration + " à " + eventObject.maxDuration + " jours"
